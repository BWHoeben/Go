...
public class GOGUIImpl extends Application {
			...
			public void handle(long currentNanoTime) {
				...
				if (x != lastX) {
					if (roundCount >= 2) {
						...
						clearBoard();
						if (x % 2 != 0) {
							drawDiagonalStoneLine(x - 1, Colour.BLACK, roundCount != 0);
							drawDiagonalStoneLine(x, Colour.WHITE, roundCount != 0);
							drawDiagonalStoneLine(x + 1, Colour.BLACK, roundCount != 0);
						}
						else {
							drawDiagonalStoneLine(x - 1, Colour.WHITE, roundCount != 0);
							drawDiagonalStoneLine(x, Colour.BLACK, roundCount != 0);
							drawDiagonalStoneLine(x + 1, Colour.WHITE, roundCount != 0);
						}
					}
	...
	private void initNewBoard() {
		...
		ImagePattern pattern = new ImagePattern(new Image("background_1920_2.jpg"));
	...
	private void initBoardLines() {
		...
		for (int i = 1; i <= height; i++) {
			...
			Line line =
					new Line(squareSize, i * squareSize, width * squareSize, i * squareSize);
		...
		for (int i = 1; i <= width; i++) {
			...
			Line line =
					new Line(i * squareSize, squareSize, i * squareSize, height * squareSize);
			line.setStroke(Color.DARKSLATEGREY);
			boardLines.add(line);
		}
	...
	private void drawDiagonalStoneLine(int diagonal, Colour stoneType, boolean flip) {
		try {
	...
	protected void addStone(int x, int y, Colour colour) throws InvalidCoordinateException {
		...
		if (mode3D){
			...
			if (colour.equals(colour.WHITE)) {
				...
				newStone.setFill(Color.DEEPPINK);
			} else if (colour.equals(colour.ORANGE)) {
				newStone.setFill(Color.DARKORANGE);
